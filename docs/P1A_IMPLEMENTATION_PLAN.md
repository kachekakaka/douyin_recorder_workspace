...
状态：**房间、FFmpeg 与安全基础已完成；真实流候选 resolver 正在按现场证据收尾，PR 仍保持 Draft。**
关联：GitHub Issue #3  
...
## 真实流候选 resolver 收尾方案（2026-07-19）
...
P1A 采用以下最小可靠链路，不把浏览器扩展为长期 RoomWorker：
...
1. 新增应用内 `DouyinStreamResolver`，先复用静态解析；只有一次性检查没有候选时才使用浏览器网络回退。
...
4. `POST /api/rooms/{room_key}/actions/check` 接入 resolver；完整候选放入有界、带过期时间的进程内缓存，进程退出即丢失，不写入数据库。
5. 先尝试通过 CDP 受限响应阶段捕获 `web-enter` JSON，解析 `live_core_sdk_data/pull_data/stream_data` 中的 FLV/HLS 和 `origin/uhd/hd/sd/ld/md`。
...
- 文档化 resolver 设计与隐私边界；
- 实现进程内 resolver 和有界候选缓存；
- 接入房间立即检查并补齐 API/SQLite 泄漏回归测试；
- 对三个授权房间执行脱敏现场回归；
- 更新 P1A 报告和 PR 描述，所有 CI 全绿后再改为 Ready for review。
